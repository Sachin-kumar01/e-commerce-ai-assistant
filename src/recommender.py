...
if page == "📊 Dashboard":
    ...
    try:
        ...
        customers = pd.read_sql(text(customer_query), engine)
        total_customers = int(customers.iloc[0]["total"])
        order_query = "SELECT COUNT(*) AS total FROM orders"
        orders = pd.read_sql(text(order_query), engine)
        ...
        revenue_query = "SELECT ROUND(SUM(revenue), 2) AS revenue FROM order_items"
        ...
        if st.button("Get Recommendations"):
            ...
            try:
                ...
                if not content_results.empty:
                    ...
                    if not collaborative_results.empty:
                        collaborative_results = collaborative_results.copy()
